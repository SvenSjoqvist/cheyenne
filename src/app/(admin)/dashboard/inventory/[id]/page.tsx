...
export default async function InventoryPage({ params }: PageProps) {
  ...
  try {
    ...
    const colorDescription = product.description ? product.description.split(' ')[0] : 'No color description available';
    ...
    const productInfoData = [
      ...
      },
      {
        ...
        value: colorDescription.length > 50 ? `${colorDescription.substring(0, 50)}...` : colorDescription
      },
      {
        label: 'Category',
    ...
    const detailedInfoData: Array<{ node: ProductDetailData }> = [{
      node: {
        ...
        productInfo: 'Material: Placeholder - Material info\nComposition: Placeholder - Composition info\nCare: Placeholder - Care instructions'
      }
    }];

    return (
      <div className="pt-16 bg-[#F7F7F7] min-h-screen px-4 h-full">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col gap-4 justify-center items-center mb-7">
            <h1 className="text-[40px] font-darker-grotesque tracking-wider font-regular">Inventory</h1>
          </div>
