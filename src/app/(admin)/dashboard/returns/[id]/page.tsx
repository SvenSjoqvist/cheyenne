...
export default async function ReturnViewPage({
  ...
  const customColumns = [
    { header: 'Return ID', accessor: 'id' as keyof (ReturnData & { allReasons: string }) },
    { header: 'Customer ID', accessor: 'customerId' as keyof (ReturnData & { allReasons: string }) },
    { header: 'Order ID', accessor: 'orderId' as keyof (ReturnData & { allReasons: string }) },
    { header: 'Date', accessor: 'createdAt' as keyof (ReturnData & { allReasons: string }) },
    { header: 'Status', accessor: 'status' as keyof (ReturnData & { allReasons: string }) },
    { header: 'Reason', accessor: 'allReasons' as keyof (ReturnData & { allReasons: string }) }
  ];
