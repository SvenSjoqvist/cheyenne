...
export default function ReturnActionForm({ returnId, customerEmail }: ReturnActionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!action) {
      setFeedback({ type: 'error', message: 'Please select either Confirm or Deny' });
      return;
    }
    ...
    try {
      ...
      await handleReturnAction(returnId, action, emailTo, message);
      
      // If we get here, the action was successful
      setFeedback({ 
        type: 'success', 
        message: `Return ${action === 'confirm' ? 'approved' : 'denied'} successfully and email sent to ${emailTo}` 
      });
      ...
      setFeedback({ 
        type: 'error', 
        message: error instanceof Error ? error.message : 'Error submitting action. Please try again.' 
      });
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-2 flex-col">
          <div className="flex items-center">
            ...
            />
            <label htmlFor="confirm" className="ml-2 text-sm font-medium text-gray-900">
              Confirm
            </label>
          </div>
          ...
          <div className="flex items-center">
            <input
              ...
              className="w-4 h-4 text-white bg-black border-black rounded focus:ring-black focus:ring-2 checked:bg-black checked:border-black"
            />
            <label htmlFor="deny" className="ml-2 text-sm font-medium text-gray-900">
              Deny
            </label>
          </div>
        </div>
