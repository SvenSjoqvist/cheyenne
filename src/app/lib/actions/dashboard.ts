...
import {
  ...
  getDashboardStats,
  getOrders,
} from "@/app/lib/shopify/admin/shopify-admin";
...
> {
  try {
    ...
    return handleApiError(error, "getAbandonedCartsCount");
  }
}
...
export async function getAverageCartValueData() {
  ...
  try {
    ...
    return {
      success: false,
      error: "Failed to fetch average cart value",
      amount: 0,
      currency: "USD",
    };
  }
...
export async function getReturnsData(): Promise<
  ApiResponse<{
    totalReturns: number;
...
> {
  try {
    ...
    return handleApiError(error, "getReturnsData");
  }
}
...
export async function getSalesData() {
  ...
  try {
    ...
    const currency =
      orders.edges[0]?.node.totalPriceSet.shopMoney.currencyCode || "USD";
    ...
    const chartData = [
      {
        ...
        revenue: monthlyRevenue[`${currentYear}-01`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-01`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-02`,
        revenue: monthlyRevenue[`${currentYear}-02`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-02`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-03`,
        revenue: monthlyRevenue[`${currentYear}-03`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-03`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-04`,
        revenue: monthlyRevenue[`${currentYear}-04`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-04`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-05`,
        revenue: monthlyRevenue[`${currentYear}-05`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-05`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-06`,
        revenue: monthlyRevenue[`${currentYear}-06`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-06`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-07`,
        revenue: monthlyRevenue[`${currentYear}-07`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-07`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-08`,
        revenue: monthlyRevenue[`${currentYear}-08`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-08`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-09`,
        revenue: monthlyRevenue[`${currentYear}-09`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-09`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-10`,
        revenue: monthlyRevenue[`${currentYear}-10`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-10`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-11`,
        revenue: monthlyRevenue[`${currentYear}-11`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-11`] || 0,
          "USD"
        ),
      },
      {
        month: `${currentYear}-12`,
        revenue: monthlyRevenue[`${currentYear}-12`] || 0,
        formattedRevenue: formatCurrency(
          monthlyRevenue[`${currentYear}-12`] || 0,
          "USD"
        ),
      },
    ];
