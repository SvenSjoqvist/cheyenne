...
async function getProduct(handle) {
  try {
    ...
    if (!accessToken) {
      console.warn('SHOPIFY_STOREFRONT_ACCESS_TOKEN not found.');
      return null;
    }
...
async function getCollections() {
  try {
    ...
    return data.data.collections.edges.map(edge => edge.node).filter(collection => collection.handle !== 'all');
 ...
 */
async function getSEODataForRoute(path) {
 ...
 */
async function generateAllRouteSEO() {
  const routes = [];
 ...
 */
async function generateSitemapWithSEO() {
